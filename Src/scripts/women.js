...
fetch("../components/layouts/women/women-menu.html")
  ...
  .then((data) => {
    ...
    const womenMenuH3 = document.querySelector("#womenMenu h3");
    window.addEventListener("scroll", function () {
      if (womenMenu.offsetTop - window.scrollY <= 0) {
        womenMenu.classList.add("py-0");
        womenMenuH3.classList.remove("text-2xl");
      }
      if (womenMenu.offsetTop - window.scrollY > 10) {
        womenMenu.classList.remove("py-0");
        womenMenuH3.classList.add("text-2xl");
      }
    });
  })
  .catch((error) => console.error("Error fetching included file:", error));
